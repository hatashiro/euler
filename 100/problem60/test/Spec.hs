...
main = hspec $ do
  ...
  describe "mergePrimeSet" $
    it "merges prime pairs if a new prime forms prime pairs for others" $ do
      mergePrimePairs (Set.fromDistinctAscList [3, 7, 109]) 673 `shouldBe` Set.fromList [3, 7, 109, 673]
      mergePrimePairs (Set.fromDistinctAscList [3, 7, 109]) 5 `shouldBe` Set.fromList [3, 7, 109]
